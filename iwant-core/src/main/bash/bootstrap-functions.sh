...
ant-bootstrapping-tutorial-targetscript() {
local TO=$1
local LOCAL_IWANT=$2
# descript needs absolute path here, since it's going to cd:
[ -n "$LOCAL_IWANT" ] && LOCAL_IWANT=$(readlink -f "$LOCAL_IWANT")
local TUTORIAL_DIR=$3
local TUTORIAL=$TUTORIAL_DIR/bootstrapping-with-ant.html
developer-script "$TO" <<EOF
#!/bin/bash
set -eu
here=\$(dirname "\$0")
...
mkdir -p "\$iwant/$TUTORIAL_DIR"
LOCAL_IWANT="$LOCAL_IWANT" bash "\$iwant/$wsroot/iwant-lib-descript/descript.sh" \\
        "\$iwant/$wsroot/iwant-docs/src/main/descript/tutorial/bootstrapping-with-ant.sh" "\$iwant/$TUTORIAL" true
echo \$iwant/$TUTORIAL
EOF
}
